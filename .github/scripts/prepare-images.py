...
def get_upstream_version(app, channel):
    latest_script = f"./hlhc/{app}/ci/latest.sh"
...
if __name__ == "__main__":
    ...
    for app in changed_apps:
        ...
        with open(f"./hlhc/{name}/metadata.json") as f:
        ...
        if isfile(f"./hlhc/{name}/{channel}/Dockerfile"):
            app["chan_context"] = f"./hlhc/{name}/{channel}"
            app["chan_dockerfile"] = f"./hlhc/{name}/{channel}/Dockerfile"
            app["chan_goss_config"] = f"./hlhc/{name}/{channel}/goss.yaml"
        else:
            app["chan_context"] = f"./hlhc/{name}"
            app["chan_dockerfile"] = f"./hlhc/{name}/Dockerfile"
            app["chan_goss_config"] = f"./hlhc/{name}/ci/goss.yaml"
